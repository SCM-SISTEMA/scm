...
if ($this->context->module->module->requestedAction->id == 'create') {
    if ($importacao['sici']->cod_sici && $importacao['sici']->situacao_fk != \app\models\TabAtributosValoresSearch::getAtributoValorAtributo('situacao-sici', 'E')) {
        $btSave = $btSave2 = $btVoltar = null;
        $js = '$("input, select").prop("disabled", true);
$("button").hide();';
    }
    $js .= " $('body').addClass('sidebar-collapse')";
