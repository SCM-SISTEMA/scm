...
class Controller extends \yii\web\Controller {
    ...
    public function retornaImportacao($rowData, $nome, $pos = false, $valor1 = 1, $valor2 = 4) {
        $i = 0;
        foreach ($rowData as $key => $value) {
               

            if ($pos) {
                if (strpos(strtoupper(trim(\projeto\Util::tirarAcentos($value[0][$valor2]))), strtoupper($nome)) !== false ||
                        strpos(strtoupper(trim(\projeto\Util::tirarAcentos($value[0][$valor1]))), strtoupper($nome)) !== false) {
                ...
                if (strtoupper(trim(\projeto\Util::tirarAcentos($value[0][$valor1]))) == strtoupper($nome)) {
                    $output = array_slice($rowData, $i);
                    return $output;
                }
            }
