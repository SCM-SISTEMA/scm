delete from comercial.tab_modelo_contrato;

INSERT INTO comercial.tab_modelo_contrato(
            cod_contrato_tipo_contrato_fk, txt_modelo)
    VALUES (498, ''),
    (499, ''),
    (500, ''),
    (501, ''),
...
update comercial.tab_modelo_contrato set txt_modelo=$$
<p align="center">
...
<p align="center" style='text-transform: uppercase'>
    <u><strong>{tipo_contrato}</strong></u>
</p>
<p align="justify">
...
<p align="justify">
    ...
    , empresa sediada na(o) {logradouro}, nº {numero}, Bairro {bairro}, no
    Município de {municipio}, no Estado {estado}, CEP: {cep}, inscrita no CNPJ
    sob o nº {cnpj}, neste ato representada por <strong>{representate_comercial}</strong>, Solteiro(a), {profissao},
    inscrita no CPF sob o nº {cpf}, telefone {telefone}, e-mail: {email},
    doravante denominada CONTRATANTE, declaram que, nesta data, ajustam entre
    si, de comum acordo e na melhor forma de direito, o presente Contrato de
    Prestação de Serviços de Consultoria Técnica para Credenciamento e Gestão
    de Obrigações SCM perante a Anatel, que será regido pela legislação vigente
...
<p align="justify">
    O contrato perdurará pelo prazo inicial de {qnt_parcelas} meses, renovando-se
...
<p align="justify">
    <a name="__DdeLink__714_421627098"></a>
    ...
    que possui valor nominal avulso de R$ {valor_isencao}.
</p>
<p align="justify">
...
<p align="justify">
    ...
    confrontando-a com sua capacidade e necessidade, optou por contratar o    <u><strong>{tipo_contrato}</strong></u> e conterá todos os serviços abaixo
    delineados:
</p>
<p align="justify">
    <strong>III.I</strong>
...
<p>
    O CONTRANTE deverá avisar com 24hrs de antecedência que será realizado o
    transbordo. Caso não seja avisada a transferência, poderão ocorrer filas no
    atendimento;
</p>
<p>
...
<p align="justify">
    ...
    CONTRATADA, através de sistema voip (voz sobre ip), sendo de obrigação da
    contratante a disponibilização de um canal voip e uma conexão estável de
    pelo menos 256 kbps durante o fluxo das ligações;
</p>
<p align="justify">
    <a name="page2"></a>
    A CONTRATADA não se responsabiliza por prejuízo em detrimento da má
    configuração ou má qualidade do equipamento da parte CONTRATANTE, bem como
    a infraestrutura utilizada. Caso necessário, a CONTRATADA poderá oferecer
    um equipamento adequado para os atendimentos, com um custo à parte.
</p>
<p align="justify">
...
<p align="justify">
    Este serviço não substitui as outras formas de atendimento e gerenciamento
    do atendimento da parte CONTRATANTE, o contrato em referência trata-se
    apenas do atendimento de primeiro nível, abertura de chamados.
</p>
<p align="justify">
    A CONTRATANTE terá acesso ao sistema por login e senha exclusiva, limitado
    a 03 (três) usuários. O custo para usuários adicionais é de R$ 50,00
    (cinquenta reais)
</p>
<p align="justify">
...
<p align="justify">
    ...
    total de <strong>R$ {valor_total_contrato}</strong>, eis que a
    CONTRATANTE aderiu ao <u style='text-transform: uppercase'><strong>{tipo_contrato}</strong></u>, pelo período
    estipulado na cláusula segunda desta avença, o qual, por mera liberalidade
    da CONTRATADA, será adimplido mediante {num_parcelas}    <strong> prestações de R$ {prestacao}</strong> cada uma,
...
<p align="justify">
    ...
    presente pacto antes de transcorrido integralmente o prazo de {qnt_parcelas} meses,
    também deverá ressarcir a CONTRATANTE na exata quantia da
    isenção que obteve em função da contratação nesta modalidade, qual seja, de
    R$ {valor_isencao}, nos moldes do previsto no
...
CREATE OR REPLACE VIEW comercial.view_cliente_contrato_all AS 
 SELECT cl.cod_cliente,
  ...
  ORDER BY c.dt_inclusao DESC, cl.dt_inclusao;
